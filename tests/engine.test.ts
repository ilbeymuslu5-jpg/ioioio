...
function headlessEngine(options: { maxTicksPerFrame?: number } = {}): GameEngine<null> {
  return new GameEngine<null>({ now: () => 0, scheduler: () => 0, context: null, ...options });
}
...
test('systems can be event-driven with attach() alone', () => {
  ...
  assert.throws(() => engine.addSystem({ name: 'empty' } as GameSystem<null>), /update\(\)/);
});

test('EventBus dispatch tolerates unsubscribing mid-emit', () => {
  const events = new EventBus<{ ping: void }>();
  const seen: string[] = [];
...
test('camera damping converges on the target and clamps to the arena', () => {
  ...
  const target = new Entity({ x: 1000, y: 1000, radius: 18 });
